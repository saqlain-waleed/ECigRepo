...
{
    public abstract class ECigWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ECigWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ECigWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<ECigDbContext> action)
        {
            using (var context = IocManager.Resolve<ECigDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<ECigDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<ECigDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ECigDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ECigDbContext, Task<T>> func)
        {
            T result;
